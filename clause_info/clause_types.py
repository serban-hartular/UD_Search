...
def is_verb(node : Tree) -> bool:
    return node._data['upos'] == 'VERB' or Search('/[deprel=cop]').find(node)

def get_previous_conjunct(node : Tree) -> Tree|None:
    if node._data['deprel'] != 'conj': return None
    parent = node.parent
...
def no_diacritics(tree : Tree) -> bool:
    for node in tree.root().traverse():
        if set('ăîâșțĂÎÂȘȚ').intersection(set(node._data['form'])):
            return False
    return True

def is_relative(node : Tree) -> str|None:
    indef_rel_prons = ['oricine', 'orice', 'oricând', 'oriunde', 'oricum', 'oricand']
    rel = Search('/[feats.PronType=Rel | _lemma=%s | /[_lemma=cum,%s] ]' % (','.join(indef_rel_prons),','.join(indef_rel_prons)))
    rel = rel.find(node)
    if rel:
...
def is_cause_effect(node : Tree) -> str|None:
    ...
    if node._data['deprel'] == 'ccomp': return None
...
def get_annotated_antecedent(ellipsis : Tree) -> Tuple[Tree|None, Tree|None]:
    antecedent_id = ellipsis._data['misc'].get('Antecedent')
    antecedent_regent_id = ellipsis._data['misc'].get('AntecedentLicenser')
    if not antecedent_id and not antecedent_regent_id:
        return (None, None)
    tree = ellipsis.root()
    if antecedent_id:
        antecedent_id = antecedent_id.pop()
        antecedent = tree.search(lambda n: n._data['id'] == antecedent_id)[0]
    else:
        antecedent = None
    if antecedent_regent_id:
        antecedent_regent_id = antecedent_regent_id.pop()
        antecedent_regent = tree.search(lambda n: n._data['id'] == antecedent_regent_id)[0]
    else:
        antecedent_regent = None
    if not antecedent_regent and antecedent._data['deprel'] in \
...
def is_paranthetic(node : Tree) -> bool:
    ...
    return (first._data['form'] == '(' and last._data['form'] == ')') or \
           (first._data['form'] == '-' and last._data['form'] == '-')
...
def is_mis_parse(node : Tree) -> str:
    ...
    next = tokens[index+1]
    if next['_lemma'] in ('să','că') or next['feats'].get('VerbForm') == {'Inf'}:
        return next['id']
    nsubj = Search('./[deprel=nsubj]').find(node)
    if nsubj:
        nsubj = nsubj[0].node
        if nsubj._data['_lemma'] in ('ce', 'ceva'):
            return nsubj._data['id']
        verb_info = vb_forms.get_verb_form(node, {'VERB'})
        if verb_info.get('Person') and verb_info.get('Person').intersection({'1','2'}) and \
                ('Person' not in nsubj._data['feats'] or not \
                        verb_info.get('Person').intersection(nsubj._data['feats']['Person'])):
            #disagreement
            return nsubj._data['id']
    return ''
